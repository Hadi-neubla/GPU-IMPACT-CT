...
!* October 2014                                                                                              *
!* modified by Hadi Zolfaghari , ARTORG CVE, then DAMTP, Cambridge University (hz382@cam.ac.uk)              *
!* Oct 2015 - Sep 2023                                                                                       *
!*************************************************************************************************************
...
!!pgi$l unroll = n:8
  ...
  SUBROUTINE export_CT_geometry
  IMPLICIT NONE

  INTEGER   ::   i,j,k

  
   CALL write_hdf('CT_geometry_large','CT_intensity',S1p,S2p,S3p,N1p,N2p,N3p,0,stride_large,CT_geometry(b1L,b2L,b3L))
...
!pgi$ unroll = n:8
  ...
  REAL(8)                   ::  old_dtime_out_scal
  REAL(8)                   ::  old_dtime_out_vect
...
!pgi$ unroll = n:8
  ...
  !-----------------------------------------------------------------------------------------------------------
         ...
         CALL read_hdf_infoREAL(1,.TRUE. ,.TRUE.,'p_end_systole_in'  ,scalar=p_end_systole_in)
         CALL read_hdf_infoREAL(1,.TRUE. ,.TRUE.,'p_end_systole_out'  ,scalar=p_end_systole_out)
         CALL read_hdf_infoREAL(1,.TRUE. ,.TRUE.,'p_end_systole_out_2'  ,scalar=p_end_systole_out_2)
  ...
  REAL(8)   , INTENT(in)    ::  phi(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U))
  ...
  CALL write_hdf_infoREAL(1     ,.TRUE. ,attr_yes,'p_end_systole_in'  ,scalar=p_end_systole_in      )
  CALL write_hdf_infoREAL(1     ,.TRUE. ,attr_yes,'p_end_systole_out' ,scalar=p_end_systole_out      )
  CALL write_hdf_infoREAL(1     ,.TRUE. ,attr_yes,'p_end_systole_out_2' ,scalar=p_end_systole_out_2      )
  ...
  REAL(8)   , INTENT(in)      ::  phi(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U),1:3)
  ...
  REAL(8)   , INTENT(in)      ::  phi(1:N1,1:N2)
  ...
  REAL(8)   , INTENT(out)     ::  phi(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U))
  ...
  REAL(8)   , INTENT(out)     ::  phi(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U)) !< requested field
  ...
  REAL(8)   , INTENT(out)     ::  phi(1:N1,1:N2)
  ...
  REAL(8)   , INTENT(in)    ::  phi(SS1:NN1, SS2:NN2, SS3:NN3, SS4:NN4)
  ...
  REAL(8)   , INTENT(out)    ::  phi(SS1:NN1, SS2:NN2, SS3:NN3, SS4:NN4)
  ...
  REAL(8)   , OPTIONAL, INTENT(in)  ::  array(1:NN)
  ...
  ELSE
     ...
     CALL H5dwrite_f(attr_id,H5T_NATIVE_DOUBLE,value,dim_mem,herror,memspace)
  ...
  ELSE
     ...
     CALL H5dwrite_f(attr_id,H5T_NATIVE_INTEGER,value,dim_mem,herror,memspace)
     CALL h5dclose_f(attr_id ,herror)
  END IF
  ...
  !> subroutine for recovering real(8) valued attribute data from hdf5 files 
  ...
  REAL(8)   , OPTIONAL, INTENT(out) ::  array(1:NN)   !< multidimensional output
  REAL(8)   , OPTIONAL, INTENT(out) ::  scalar        !< onedimensional output
  ...
  ELSE
     ...
     IF (rank == 0) CALL H5dread_f(attr_id,H5T_NATIVE_DOUBLE,value,dim_mem,herror)
     CALL h5dclose_f(attr_id,herror)
  END IF
  ...
  ELSE
     CALL h5dopen_f(file_id,name,attr_id,herror)
     IF (rank == 0) CALL H5dread_f(attr_id,H5T_NATIVE_INTEGER,value,dim_mem,herror)
     CALL h5dclose_f(attr_id,herror)
  END IF
...
!pgi$ unroll = n:8
  ...
  SUBROUTINE write_3D_xdmf(filebase, stride_string, count_char, dim1, dim2, dim3)
    ...
    CHARACTER(LEN=5), INTENT(in)  ::  count_char
      ...
      IF (dimens .EQ. 3) THEN
        IF (write_large) THEN
          ...
          DO i= 0, write_count-1
            CALL num_to_string(5, i, count_char)
