...
!! @brief File in which all the containers (scalar fields, vector fields, coordinates ...) are allocated.
  ...
  !===== CT-based DNS
  !==================================
  ALLOCATE(x_global(1:M1))
  ALLOCATE(y_global(1:M2))
  ALLOCATE(z_global(1:M3))

  ALLOCATE(ct_grid(1:(M1),1:(M2),1:(M3)))
!  ALLOCATE(ct_grid_per_process(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U)))
  ...
  !--- C-FORTRAN interface ---
  x1pptr = C_LOC(x1p(b1L))
  x2pptr = C_LOC(x2p(b2L))
  x3pptr = C_LOC(x3p(b3L))

  x1uptr = C_LOC(x1u(b1L))
  x2vptr = C_LOC(x2v(b2L))
  x3wptr = C_LOC(x3w(b3L))
  ...
  ALLOCATE(dy3w(0:M3))
  !--- C-FORTRAN interface ---
  dy1pptr = C_LOC(dy1p(1))
  dy2pptr = C_LOC(dy2p(1))
  dy3pptr = C_LOC(dy3p(1))

  dy1uptr = C_LOC(dy1u(0))
  dy2vptr = C_LOC(dy2v(0))
  dy3wptr = C_LOC(dy3w(0))
  ...
  !--- C-FORTRAN interface --
  dx1pptr = C_LOC(dx1p(1))
  dx2pptr = C_LOC(dx2p(1))
  dx3pptr = C_LOC(dx3p(1))

  dx1uptr = C_LOC(dx1u(0))
  dx2vptr = C_LOC(dx2v(0))
  dx3wptr = C_LOC(dx3w(0))
  ...
  !--- C-FORTRAN interface ---
  velptr = C_LOC(vel(b1L,b2L,b3L,1))

  !--- nicht-linearer Term -----------------------------------------------------------------------------------
  ALLOCATE(nl (b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U),1:3))
  !--- C-FORTRAN interface ---
  nlptr = C_LOC(nl(b1L,b2L,b3L,1))

  !--- Recht-Hand-Seite --------------------------------------------------------------------------------------
  ALLOCATE(rhs(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U),1:3))
  !--- C-FORTRAN interface ---
  rhsptr = C_LOC(rhs(b1L,b2L,b3L,1))
  
  !--- Druck -------------------------------------------------------------------------------------------------
  ALLOCATE(pre(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U)))
  !--- C-FORTRAN interface ---
  preptr = C_LOC(pre(b1L,b2L,b3L))
   
  !--- local (on-process) container for the geometric CT data
  ALLOCATE(CT_geometry(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U)))
  ...
  !--- C-FORTRAN interface ---
  work1ptr = C_LOC(work1(b1L,b2L,b3L))
  work2ptr = C_LOC(work2(b1L,b2L,b3L))
  work3ptr = C_LOC(work3(b1L,b2L,b3L))
