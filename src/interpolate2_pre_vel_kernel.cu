...
// !* April 2020 -                                                                                   *            
// !**************************************************************************************************
extern "C" __global__ void interpolate_kernel(int dimmension_id, int nx, int ny, int nz, int nxI, int nyI, double* phi, double* cI, double* inter)
...
////                if ((k >= 4) && (k <= nz-5)) {
        ...
        inter[index1] = cI[(i)*nxI] * phi[index1-2] ;
...
//                                                      + phi[ ( (k+4) * nx * ny + (j+4) * nx + (i+4) + 5 - 2) ] * cI[(i) * nxI + 5];
        for (int l=1; l<= 5; l++){
              inter[index1]  = inter[index1] + phi[ ( (k+4) * nx * ny + (j+4) * nx + (i+4) + l - 2) ] * cI[(i) * nxI + l];
        }
...
////                if ((k >= 4) && (k <= nz-5)) {
        ...
        inter[index1] = cI[(j)*nxI] * phi[index1- 2 * nx];
...
//                                                            + phi[ ( (k+4) * nx * ny + ((j+4) + 5 - 2) * nx + (i+4)) ] * cI[(j) * nxI + 5];
        for (int l=1; l<= 5; l++){
            ...
            inter[index1] = inter[index1] + phi[ ( (k+4) * nx * ny + ((j+4) + l - 2) * nx + (i+4)) ] * cI[(j) * nxI + l];
        }
//         inter[index1] += phi[ ( k * nx * ny + (j + l - 2) * nx + i) ] * cI[(j-4) * nxI + l] ;
...
////                if ((k >= 4) && (k <= nz-5)) {
       ...
       k += 4;
        ...
        inter[index1] = cI[(k-4)*nxI] * phi[index1- 2 * nx * ny];
...
//                                                                 + phi[ ( (k + 5 -2) * nx * ny + j * nx + i ) ] * cI[(k-4) * nxI + 5] ;
        for (int l=1; l<= 5; l++){
         inter[index1] = inter[index1] + phi[ ( (k + l -2) * nx * ny + j * nx + i ) ] * cI[(k-4) * nxI + l]; 
        }
