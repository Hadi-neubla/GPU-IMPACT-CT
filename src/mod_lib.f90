...
!!pgi$l unroll = n:8
  ...
  REAL(8)                   ::  dd1
  REAL(8)                   ::  alpha_re, alpha_im
  REAL(8)                   ::  beta(1:2), beta_global(1:2)
  REAL(8)                   ::  angle, radius
  
  REAL(8)                   ::  kdx_max_conv(1:3)
  REAL(8)                   ::  kdx_max_visc(1:3)
  
  REAL(8)                   ::  array_float(1:2)
  LOGICAL                ::  array_log (1:4)
  
  REAL(8)                   ::  newtime
  REAL(8)                   ::  pi
  
  REAL(8)                   ::  f_max, p_per_side !bbecsek 150510
  !----------------------------------------------------------------------------------------------------------!
  ...
  IF ((Int_dtime /= 0 .AND. MOD(timestep,Int_dtime) == 0) .OR. new_dtime) THEN
     ...
     IF (rank == 0) THEN ! TEST!!! ok?
        ...
        ! bbecsek 150507 IB heuristics (beam Eigenfrequency dependent)
        ! the factor U_ref in front of the square root should be thought of as a reference velocity, but can be used for scaling
        IF(IB_ON) THEN
        
          IF(dtime .GT. 2.*pi/max_ev_freq) dtime = 2.*pi/max_ev_freq
          !p_per_side = MAXVAL(xb(:,2))/(xb(2,1)-xb(1,1)) + 1
          
          !IF (dtime .GT.  0.9*mu_blood * Re * ((L_ref*sqrt( E_mod * L3 * rho_blood / 12.) * (p_per_side/2.)**2.)**(-1.))) THEN
          !  dtime = 0.9*mu_blood * Re * ((L_ref*sqrt( E_mod * L3 * rho_blood / 12.) * (p_per_side/2.)**2.)**(-1.))
          !END IF
!write(*,*) 'max||k_stiff||= ',U_ref*SQRT( rho_blood/MAXVAL(ABS(k_stiff)) )*(8./M_bound)**2
  ...
  IF (rank == 0) THEN
     ...
     array_float = (/time,dtime/)
     array_log  = (/finish_yes,new_dtime,write_out_vect,write_out_scal/)
  END IF
  
  CALL MPI_BCAST(array_float,2,MPI_REAL8  ,0,COMM_CART,merror)
...
!pgi$ unroll = n:8
     ...
     pre0 = pre0_global/REAL(dim1)/REAL(dim2)/REAL(dim33) ! TEST!!! wegen i4 gefaehrlich!
  ...
  REAL(8)                ::  countO_av
  REAL(8)                ::  countP_av(1:2)
  REAL(8)                ::  countH_av(1:3)
  REAL(8)                ::  countP_all_av
  REAL(8)                ::  countH_all_av
