...
!      IF (concentration_yes) CLOSE(24)
  ...
  REAL(8)                   ::  u_mod       (1:2,1:3,-amax:amax,0:bmax)
  REAL(8)                   ::  u_mod_global(1:2,1:3,-amax:amax,0:bmax)
  
  REAL(8)                   ::  TKE_mod       (-amax:amax,0:bmax)
  REAL(8)                   ::  TKE_mod_global(-amax:amax,0:bmax)
  
  REAL(8)                   ::  TKE(1:2), TKE_global(1:2)
  
  REAL(8)                   ::  mean       (1:3,S1p:N1p), covar       (1:6,S1p:N1p), diss_visc       (S1p:N1p)
  REAL(8)                   ::  mean_global(1:3,S1p:N1p), covar_global(1:6,S1p:N1p), diss_visc_global(S1p:N1p)
  REAL(8)                   ::  mean_write (1:3,1  :M1 ), covar_write (1:6,1  :M1 ), diss_visc_write (1  :M1 )
  REAL(8)                   ::  deriv      (1:3,1:3)
  
  REAL(8)                   ::  diss, diss_viscInt, diss_viscInt_global
