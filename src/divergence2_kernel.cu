...
// !* April 2020 -                                                                                   *            
// !**************************************************************************************************
extern "C" __global__ void divergence2_kernel( int nx, int ny, int nz, int nxS1, int nxS2,int nxS3, double* phi, double* div, double* cS1, double* cS2, double* cS3)
...
////                if ((k >= 4) && (k <= nz-5)) {
        ...
        int index1 = (i+4) + (j+4) * (nx) + (k+4) * (nx) * (ny);
         ...
         div [index1] = cS1[(1 + i) * nxS1] * phi[index1 - 3] ;
...
////-- looped implementation 
        for (int l=1; l<= 5; l++){
             div[index1]  = div[index1] + phi[ ( (k+4) * nx * ny + (j+4) * nx + (i+4) + l - 3) ] * cS1[(i + 1) * nxS1 + l];
        }

        for (int l=0; l<= 5; l++){
            div[index1] = div[index1] + phi[nx * ny * nz + ( (k+4) * nx * ny + ((j+4) + l -3) * nx + (i + 4)) ] * cS2[(j + 1) * nxS2 + l];
        }

         for (int l=0; l<= 5; l++){
             div[index1] = div[index1] + phi[2 * nx * ny * nz + ( (k  + 4 + l -3) * nx * ny + (j+4) * nx + (i+4) ) ] * cS3[(k + 1) * nxS3 + l]; 
        }
