...
!   REAL   , target, INTENT(  out) ::  comp(b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< gradient operator
...
!   REAL   , target ::  west_ghost(1:N2-1,1:N3-1)
!   REAL   , target ::  east_ghost(1:N2-1,1:N3-1)
!   REAL   , target ::  front_ghost(1:N1-1,1:N2-1)
!   REAL   , target ::  rear_ghost(1:N1-1,1:N2-1)
!   REAL   , target ::  lower_ghost(1:N1-1,1:N3-1)
!   REAL   , target ::  upper_ghost(1:N1-1,1:N3-1)
!
!
!   REAL   , target ::  west_chunk(1:N2-1,1:N3-1)
!   REAL   , target ::  east_chunk(1:N2-1,1:N3-1)
!   REAL   , target ::  front_chunk(1:N1-1,1:N2-1)
!   REAL   , target ::  rear_chunk(1:N1-1,1:N2-1)
!   REAL   , target ::  lower_chunk(1:N1-1,1:N3-1)
!   REAL   , target ::  upper_chunk(1:N1-1,1:N3-1)





!   REAL   , target ::  slide_south(b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 
!   REAL   , target ::  slide_north(b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 
...
!  type(cudaStream_t) :: stream1, stream2, stream3
  ...
  real(8)     ::  pre_inlet_multiplied, pre_outlet_multiplied, pre_outlet_2_multiplied
...
!----------------------------------------------------------------
    ...
    pre_inlet_multiplied = pre_inlet *  (aRK(substep)+bRK(substep)) * dtime
    pre_outlet_multiplied = pre_outlet *  (aRK(substep)+bRK(substep)) * dtime
    pre_outlet_2_multiplied = pre_outlet_2 *  (aRK(substep)+bRK(substep)) * dtime
    if (substep == 3) CALL pressure_bc_launcher(1, 1, 0, k_start_index, nx_dev,ny_dev,nz_dev, pre_inlet_multiplied, pre_outlet_multiplied, pre_outlet_2_multiplied, ct_geometry_dev, rel_dev)  
...
!    endif 
    ...
    call cuda_err_check(istat)
     ...
     ! ----- pressure BC block
    !if (substep == 3)  CALL apply_pressure_BC_CT(rel)
    !istat = cudaMemcpy(rel_dev, c_loc(rel), nx_dev * ny_dev * nz_dev * szreal, cudaMemcpyHostToDevice)
    !call cuda_err_check(istat) 
...
!        if (rank ==0 ) print*, iter, norm_inf
       ...
       if (rank ==0 ) print*, iter, mode_j*8.*0.8*3.1415*3.1415/(M1*M1), err_tot, rel_err_tot, norm_inf
       !if (rel_err_tot .le. (1*10.0e-5)) EXIT ITERATE
       IF (rel_err_tot .le. (20.*mode_j*8.*1./3.0*0.8*3.1415*3.1415*(1./(M1*M1)+1./(M2*M2)+1./(M3*M3)))) THEN
          !call apply_pressure_BC_CT(rel)
          EXIT ITERATE
       ENDIF
