...
!!pgi$l unroll = n:8
  ...
  !GPU acceleration mode
    IF (INDEX(text,'GPU_accelerated'                        ) == 1) READ(UNIT=text,FMT=*) dummy, GPU_accelerated
    IF (INDEX(text,'GPU_Poisson_Solver'                        ) == 1) READ(UNIT=text,FMT=*) dummy, GPU_Poisson_Solver
    IF (INDEX(text,'GPU_verbose'                        ) == 1) READ(UNIT=text,FMT=*) dummy, GPU_verbose 
  ...
  !===========================================================================================================
  !--- Switch immersed boundary ON/OFF ---
  !IB_on = .TRUE.
    IF (INDEX(text,'IB_on'                       ) == 1) READ(UNIT=text,FMT=*) dummy, IB_on

  !--- dynamic viscosity of blood (or other fluid) [Pa*s] Literature Value: ~0.01
  !mu_blood = 0.01
    IF (INDEX(text,'mu_blood'                    ) == 1) READ(UNIT=text,FMT=*) dummy, mu_blood

  !--- bulk density of blood (or other fluid) [kg/m3]
  !rho_blood = 1.
    IF (INDEX(text,'rho_blood'                   ) == 1) READ(UNIT=text,FMT=*) dummy, rho_blood

  !--- Young's modulus of FEM solid (linear Hooke's Law holds, plane stress) [Pa]
  !E_mod = 5.*10.**10.
    IF (INDEX(text,'E_mod'                       ) == 1) READ(UNIT=text,FMT=*) dummy, E_mod

  !--- Poisson's ratio (for isotropic materials usually 0.2 < nu < 0.5, 0.5 incompressible) [-]
  !nu_poiss = 0.499
    IF (INDEX(text,'nu_poiss'                    ) == 1) READ(UNIT=text,FMT=*) dummy, nu_poiss

  !--- delta kernel function type ---
  !- 1: cosine function with 2h support (Peskin 1977, 2000, ...)
  !- 2: hat function with 2h support  (Saiki & Biringen 1996)
  !- 3: smooth function with preservation of spatial scheme accuracy with 2h support (Beyer & Leveque 1992)
  !- 4: smooth function with 2h support (Lai & Peskin 2000)
  !ddf_type = 4
    IF (INDEX(text,'ddf_type'                    ) == 1) READ(UNIT=text,FMT=*) dummy, ddf_type

  !--- Reach of interpolation and spreading --- (rename?)
  !reach = 1./32.
    IF (INDEX(text,'reach'                       ) == 1) READ(UNIT=text,FMT=*) dummy, reach
  ...
  !--- density of solid --- [kg/m3]  
    IF (INDEX(text,'rho_solid'                   ) == 1) READ(UNIT=text,FMT=*) dummy, rho_solid
  ...
  !--- Inertance --- [Pa * s2/m3]
    IF (INDEX(text,'L_blood'                     ) == 1) READ(UNIT=text,FMT=*) dummy, L_blood
  ...
  !--- Flag whether to use FEM or not ---
    IF (INDEX(text,'fem_yes'                   ) == 1) READ(UNIT=text,FMT=*) dummy, fem_yes

  !--- Spring constants ---
    IF (INDEX(text,'k_x'                       ) == 1) READ(UNIT=text,FMT=*) dummy, k_x
    IF (INDEX(text,'k_y'                       ) == 1) READ(UNIT=text,FMT=*) dummy, k_y
    IF (INDEX(text,'k_z'                       ) == 1) READ(UNIT=text,FMT=*) dummy, k_z

  !--- Aorta Dimensions ---
    IF (INDEX(text,'aorta_start'               ) == 1) READ(UNIT=text,FMT=*) dummy, aorta_start
    IF (INDEX(text,'aorta_end'                 ) == 1) READ(UNIT=text,FMT=*) dummy, aorta_end
    IF (INDEX(text,'aorta_radius'              ) == 1) READ(UNIT=text,FMT=*) dummy, aorta_radius
    

  !--- boundary points to exclude from computation ---
  !excl_p = (/1, 2*M_bound/3, 2*M_bound/3+1/) ! M_bound is not known here yet
