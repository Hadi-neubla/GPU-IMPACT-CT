...
implicit none
   ...
   real(8) , target, intent(inout)     ::  omega
   real(8)*4 , target                          :: omega_sp

   real(8)*4, target       :: cdg1_sp (-1:1,1:N1,1:n_grids_max)
   real(8)*4, target       :: cdg2_sp (-1:1,1:N2,1:n_grids_max)
   real(8)*4, target       :: cdg3_sp (-1:1,1:N3,1:n_grids_max)
   ...
   REAL(8)*4  , target::  rel_sp (b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 
   REAL(8)*4  , target ::  bb_sp (b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 


  ! REA , target, INTENT(out) ::  comp (b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 

   REAL(8)*4  , target ::  rel_old(b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 
   REAL(8)*4  , target ::  GPUres(1:N1p,1:N2p,1:N3p)!< ? 
...
!  type(c_ptr) :: d_x, d_y, d_xy
  ...
  integer(c_size_t) :: szreal, szreal8, szint, szptr, offset = 0
...
!  type(cudaStream_t) :: stream1, stream2, stream3
  ...
  real(8)*4     ::  norm_inf, norm_inf2, norm_inf_global, norm_inf_global2, norm_zero
  real(8)     ::  t1, t2, t3
  include 'mpif.h'
...
!   CALL exchange(dimension_id,0,phi) 
  ...
  !------------------------- casting the double to single precision -----------------------------
  !----------------------------------------------------------------------------------------------
  ...
  szreal = sizeof(rel_sp(1, 1, 1))
  szreal8 = sizeof(vel(1,1,1,1))
  szint = sizeof(nx)
...
!    threads = dim3(1, 1, 1)
    ...
    istat = cudaMemcpy(cdg1_dev, c_loc(cdg1_sp), nx_cdg1 * ny_cdg1 * nz_cdg1 * szreal, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)
    istat = cudaMemcpy(cdg2_dev, c_loc(cdg2_sp), nx_cdg2 * ny_cdg2 * nz_cdg2 * szreal, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)
    istat = cudaMemcpy(cdg3_dev, c_loc(cdg3_sp), nx_cdg3 * ny_cdg3 * nz_cdg3 * szreal, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)
    
    endif


    istat = cudaMemcpy(bb_dev, c_loc(bb_sp), nx * ny * nz * szreal, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)

    istat = cudaMemcpy(rel_dev, c_loc(rel_sp), nx * ny * nz * szreal, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)

    !----------- initialize the ghosts
    west_ghost  = rel_sp(S1R-1,1:N2-1,1:N3-1)
    east_ghost  = rel_sp(N1R+1,1:N2-1,1:N3-1)
    lower_ghost = rel_sp(1:N1-1,S2R-1,1:N3-1)
    upper_ghost = rel_sp(1:N1-1,N2R+1,1:N3-1) 
    rear_ghost  = rel_sp(1:N1-1,1:N2-1,S3R-1)
    front_ghost = rel_sp(1:N1-1,1:N2-1,N3R+1)

    west_chunk  = rel_sp(S1R,1:N2-1,1:N3-1)
    east_chunk  = rel_sp(N1R,1:N2-1,1:N3-1)
    lower_chunk = rel_sp(1:N1-1,S2R,1:N3-1)
    upper_chunk = rel_sp(1:N1-1,N2R,1:N3-1) 
    rear_chunk  = rel_sp(1:N1-1,1:N2-1,S3R)
    front_chunk = rel_sp(1:N1-1,1:N2-1,N3R)
...
!----------------------------------------------------------------
    ...
    istat = cudaMemcpy(c_loc(rel_sp), rel_dev, nx * ny * nz * szreal, cudaMemcpyDeviceToHost)
    call cuda_err_check(istat)
 !   print*, rel(5,5,5)

     CALL exchange_relax_GPU_sp(1,0,0,0,0,.FALSE.,rel_sp)
      ...
      norm_inf2= 0.
       ...
       DO k = SS3, NN3
          DO j = SS2, NN2
             DO i = SS1, NN1
                GPUres(i,j,k) = bb_sp(i,j,k)                                     &
                            &      - cdg1_sp(-1,i,1)*rel_sp(i-1,j,k) - cdg1_sp(1,i,1)*rel_sp(i+1,j,k)     &
                            &      - cdg2_sp(-1,j,1)*rel_sp(i,j-1,k) - cdg2_sp(1,j,1)*rel_sp(i,j+1,k)     &
                            &      - cdg3_sp(-1,k,1)*rel_sp(i,j,k-1) - cdg3_sp(1,k,1)*rel_sp(i,j,k+1)    &
                            &      - (cdg1_sp(0,i,1) + cdg2_sp(0,j,1) + cdg3_sp(0,k,1))*rel_sp(i,j,k)
                norm_inf = MAX(ABS(GPUres(i,j,k)),norm_inf)
                norm_inf2 = MAX(ABS(rel_sp(i,j,k)-rel_old(i,j,k)),norm_inf2)
               rel_old(i,j,k) = rel_sp(i,j,k)
             END DO
          END DO
       END DO
...
!    end if
    ...
    !--------------------------------------------------------------------------------------------------------
       CALL MPI_ALLREDUCE(norm_inf,norm_inf_global,1,MPI_REAL(8)4,MPI_MAX,COMM_CART,merror) ! MPI_REDUCE bringt nichts, weil exit_yes dann mit MPI_BCAST verteilt werden müsste ...
       norm_inf = norm_inf_global
    !   if (iter == 1) norm_zero = norm_inf 


       CALL MPI_ALLREDUCE(norm_inf2,norm_inf_global2,1,MPI_REAL(8)4,MPI_MAX,COMM_CART,merror) ! MPI_REDUCE bringt nichts, weil exit_yes dann mit MPI_BCAST verteilt werden müsste ...
       norm_inf2 = norm_inf_global2
     !========================================================================================================
        if (rank ==0 ) print*, iter, norm_inf, norm_inf2
        if (norm_inf .le. 5.e-2) then
               EXIT ITERATE
              rel= dble(rel_sp) 
        endif
   ...
   ! Setup CUDA compute grid configuration.
    ...
    !!! Stream 1 : the cuda API break down
       ...
       ! Previous arguments were 4 bytes in size, total offset is 12 bytes.
        ...
        ! (from 12 to 16).
    !    offset = offset + 4
        istat = cudaSetupScalarArgument(c_loc(omega_sp), szreal, offset)
        call cuda_err_check(istat)
...
!        t1 =   MPI_Wtime()
        ...
        istat = cudaLaunch('jor_kernel_v2_sp' // c_null_char)
...
!        t1 =   MPI_Wtime()
        ...
        istat = cudaLaunch('get_ghost_sp' // c_null_char)
