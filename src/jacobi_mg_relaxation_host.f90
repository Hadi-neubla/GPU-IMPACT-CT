...
!**************************************************************************************************
subroutine device_jacobi_mg_relaxation(grid_id,omega_g,rel_g,bb_g, comp_g)
...
implicit none
   ...
   real(8), target, intent(in)     ::  omega_g

   REAL(8)   , target, INTENT(inout) ::  rel_g (b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 
   REAL(8)   , target, INTENT(inout) ::  bb_g (b1L:(NN(1,grid_id)+b1U),b2L:(NN(2,grid_id)+b2U),b3L:(NN(3,grid_id)+b3U))!< ? 
...
!  type(c_ptr) :: d_x, d_y, d_xy
  type(c_ptr) :: rel_dev3, bb_dev3, comp_dev3, cdg1_dev3,  cdg2_dev3, cdg3_dev3
...
!    call cpu_time(start)
    ...
    ! Get the size of array passing to jacobi relaxation -- > ref : mod_laplace
...
!    print*, nx, ny, nz, nx_cdg1, nx_cdg2, nx_cdg3, ny_cdg1, ny_cdg2, ny_cdg3, nz_cdg1, nz_cdg2, nz_cdg3, 'nx, ny, nz, nx_cdg1, nx_cdg2, nx_cdg3, ny_cdg1, ny_cdg2, ny_cdg3, nz_cdg1, nz_cdg2, nz_cdg3'
    ...
    istat = cudaMalloc(cdg1_dev3, nx_cdg1_g * ny_cdg1_g * nz_cdg1_g * szreal_g)
    call cuda_err_check(istat)

    istat = cudaMalloc(cdg2_dev3, nx_cdg2_g * ny_cdg2_g * nz_cdg2_g * szreal_g)
    call cuda_err_check(istat)

    istat = cudaMalloc(cdg3_dev3, nx_cdg3_g * ny_cdg3_g * nz_cdg3_g * szreal_g)
    call cuda_err_check(istat)  
 


    istat = cudaMalloc(rel_dev3, nx_dev_g * ny_dev_g * nz_dev_g * szreal_g)
    call cuda_err_check(istat)
    ! rel, bb and comp come in same size -- ref to mod_laplace
    istat = cudaMalloc(bb_dev3, nx_dev_g * ny_dev_g * nz_dev_g *  szreal_g)
    call cuda_err_check(istat)
    istat = cudaMalloc(comp_dev3, nx_dev_g * ny_dev_g * nz_dev_g *  szreal_g)
    call cuda_err_check(istat)
...
!    threads = dim3(1, 1, 1)
    ...
    istat = cudaMemcpy(cdg1_dev3, c_loc(cdg1), nx_cdg1_g * ny_cdg1_g * nz_cdg1_g * szreal_g, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)
    istat = cudaMemcpy(cdg2_dev3, c_loc(cdg2), nx_cdg2_g * ny_cdg2_g * nz_cdg2_g * szreal_g, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)
    istat = cudaMemcpy(cdg3_dev3, c_loc(cdg3), nx_cdg3_g * ny_cdg3_g * nz_cdg3_g * szreal_g, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)
    

    istat = cudaMemcpy(rel_dev3, c_loc(rel_g), nx_dev_g * ny_dev_g * nz_dev_g * szreal_g, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)

    istat = cudaMemcpy(bb_dev3, c_loc(bb_g), nx_dev_g * ny_dev_g * nz_dev_g * szreal_g, cudaMemcpyHostToDevice)
    call cuda_err_check(istat)

!!    istat = cudaMemcpy(comp_dev3, c_loc(comp), nx * ny * nz * szreal, cudaMemcpyHostToDevice)
!!    call cuda_err_check(istat)
   ...
   ! Setup CUDA compute grid configuration.
    ...
    !!! Stream 1 : the cuda API break down
       ...
       ! Previous arguments were 4 bytes in size, total offset is 12 bytes.
        ...
        ! (from 12 to 16).
    !    offset = offset + 4
        istat = cudaSetupScalarArgument(c_loc(omega_g), szreal_g, offset)
        call cuda_err_check(istat)
        offset = offset + sizeof(omega)
        istat = cudaSetupArrayArgument(cdg1_dev3, szptr_g, offset)
        call cuda_err_check(istat)
        offset = offset + sizeof(cdg1_dev3)
        istat = cudaSetupArrayArgument(cdg2_dev3, szptr_g, offset)
        call cuda_err_check(istat)
        offset = offset + sizeof(cdg2_dev3)
        istat = cudaSetupArrayArgument(cdg3_dev3, szptr_g, offset)
        call cuda_err_check(istat)
        offset = offset + sizeof(cdg3_dev3)
        istat = cudaSetupArrayArgument(rel_dev3, szptr_g, offset)
        call cuda_err_check(istat)
        offset = offset + sizeof(rel_dev3)
        istat = cudaSetupArrayArgument(bb_dev3, szptr_g, offset)
        call cuda_err_check(istat)
        offset = offset + sizeof(bb_dev3)
        istat = cudaSetupArrayArgument(comp_dev3, szptr_g, offset)
        call cuda_err_check(istat)
        offset = offset + sizeof(comp_dev3)
...
!        t1 =   MPI_Wtime()
        ...
        istat = cudaLaunch('jacobi_kernel' // c_null_char)
...
!        print*, nx, ny, nz, 'compute time ', t2-t1
  ...
  call jacobi_launcher((grid_id), (nx_dev_g), (ny_dev_g),(nz_dev_g), (ny_cdg1_g), (ny_cdg2_g), ny_cdg3_g, nx_cdg1_g,omega_g,cdg1_dev3,cdg2_dev3,cdg3_dev3, (rel_dev3), (bb_dev3), comp_dev3)




    ! Copy back results from GPU to CPU memory.
    istat = cudaMemcpy(c_loc(comp_g), comp_dev3, nx_dev_g * ny_dev_g * nz_dev_g * szreal_g, cudaMemcpyDeviceToHost)
    call cuda_err_check(istat)



    ! Free allocated GPU memory.
    istat = cudaFree(rel_dev3)
    call cuda_err_check(istat)
    istat = cudaFree(bb_dev3)
    call cuda_err_check(istat)
    istat = cudaFree(comp_dev3)
    call cuda_err_check(istat)
    istat = cudaFree(cdg1_dev3)
    call cuda_err_check(istat)
    istat = cudaFree(cdg2_dev3)
    call cuda_err_check(istat)
    istat = cudaFree(cdg3_dev3)
    call cuda_err_check(istat)


! Applying boundary conditions

end subroutine device_jacobi_mg_relaxation
