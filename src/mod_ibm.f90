...
MODULE mod_ibm
  ...
  INTEGER                :: dof_red_len
  REAL(8), ALLOCATABLE      :: m_mass_red(:,:), k_stiff_red(:,:)
  REAL(8), ALLOCATABLE      :: alpha_r(:), alpha_i(:), beta_r(:), work_ev(:)
  ...
  REAL(8)                   :: delta_z, l_half, phi_aorta, theta_aorta
  ...
  !=== initialize boundary ====================================================================================
  !*** deprecated *********************************************************************************************
  !delta_b = L2/(2.*(infoREALM_bound)/3. - 1.))
  ...
  !    yb(i,2) = infoREALi-1)*(delta_b)
  !    yb(i,1) = L1 - 2.5
  !  ELSE IF (i .GT. M_bound/3 .AND. i .LE. 2*M_bound/3) THEN
  !    yb(i,2) = infoREAL2*M_bound/3-i)*(delta_b)
  !    yb(i,1) = L1 - 2.5 + (delta_b)
  !  ELSE IF (i .GT. 2*M_bound/3) THEN
  !    yb(i,2) = infoREALi-1-2*M_bound/3)*(delta_b)
...
!         alpha_r, alpha_i, beta_r, DUMMY_EV, 1, DUMMY_EV, 1, work_ev, 16*(M_bound-dof_red_len), ev_info)
  ...
  SUBROUTINE spread_force_dens_to_vel_grid
    ...
    REAL(8)   , DIMENSION(:), ALLOCATABLE  :: dist_1p, dist_2p, dist_3p, dist_1u, dist_2v, dist_3w
  ...
  SUBROUTINE interpolate_vel_to_ib
    ...
    INTEGER, DIMENSION(2)              :: ind_1p, ind_2p, ind_3p, ind_1u, ind_2v, ind_3w
    REAL(8)   , DIMENSION(:), ALLOCATABLE :: dist_1p, dist_2p, dist_3p, dist_1u, dist_2v, dist_3w
    REAL(8)                               :: u1_reduced, u2_reduced, u3_reduced, u1_forsend, u2_forsend, u3_forsend
  ...
  REAL(8),    INTENT(OUT),DIMENSION(Eul_ind_1p(2) - Eul_ind_1p(1) + 1)           :: dist_1p
  REAL(8),    INTENT(OUT),DIMENSION(Eul_ind_2p(2) - Eul_ind_2p(1) + 1)           :: dist_2p
  REAL(8),    INTENT(OUT),DIMENSION(Eul_ind_3p(2) - Eul_ind_3p(1) + 1), OPTIONAL :: dist_3p
  REAL(8),    INTENT(OUT),DIMENSION(Eul_ind_1u(2) - Eul_ind_1u(1) + 1)           :: dist_1u
  REAL(8),    INTENT(OUT),DIMENSION(Eul_ind_2v(2) - Eul_ind_2v(1) + 1)           :: dist_2v
  REAL(8),    INTENT(OUT),DIMENSION(Eul_ind_3w(2) - Eul_ind_3w(1) + 1), OPTIONAL :: dist_3w
  ...
  SUBROUTINE calculate_node_volumes(elem, elem_vol, f1, f2, f3, g1, g2, g3, h1, h2, h3, x1, x2, x3, y1, y2, y3)
    ...
    REAL(8)                        :: xx1, xx2, xx3, yy1, yy2, yy3
    REAL(8)                        :: ff1, ff2, ff3, gg1, gg2, gg3, hh1, hh2, hh3
    REAL(8)                        :: A
    
    INTEGER,INTENT(IN)          :: elem    
    REAL(8),INTENT(OUT),OPTIONAL   :: elem_vol
    REAL(8),INTENT(OUT),OPTIONAL   :: f1, f2, f3, g1, g2, g3, h1, h2, h3, x1, x2, x3, y1, y2, y3
  ...
  SUBROUTINE update_boundary
    ...
    !ub = ub*(mu_blood*Re/(rho_blood*U_ref)) ! bbecsek 150507: make dimensional
  ...
  SUBROUTINE setup_stiffness_matrix
    ...
    REAL(8)                 :: A
    REAL(8), DIMENSION(3,6) :: B_transp
    REAL(8), DIMENSION(6,6) :: k_stiff_local
    REAL(8)                 :: k_forsend, k_reduced
...
!                            + B_transp(3,6)*( C(3,1)*B_transp(1,6) + C(3,2)*B_transp(2,6) + C(3,3)*B_transp(3,6) ) )
 ...
 !! the triangular 3-node elements are known for general cases.
  SUBROUTINE setup_mass_matrix
    ...
    REAL(8)                 :: A
    REAL(8), DIMENSION(6,6) :: m_mass_local
  ...
  FUNCTION gq_int(b1, c1, d1, b2, c2, d2, x1, x2, x3, y1, y2, y3, A) RESULT(fn_val)
    ...
    REAL(8)   , INTENT(IN) :: b1, c1, d1, b2, c2, d2
    REAL(8)   , INTENT(IN) :: x1, x2, x3, y1, y2, y3, A
    REAL(8)                :: fn_val
